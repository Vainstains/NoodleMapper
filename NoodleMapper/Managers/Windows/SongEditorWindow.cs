    ...
    protected override void BuildUI()
    {
        ...
        foreach (var c in characteristics)
        {
            ...
            foreach (var d in c.Value)
            {
                ...
                bool isNMEnabled = diffInfo.CustomData.TryGetString(JsonKeys.MapFile, out var mapFile);

                // if (isNMEnabled)
                //     mapRect.AddBorder(RectTransform.Edge.Right);

                mapRect.Field("Enable NM", 1, -26).AddToggle().SetValue(isNMEnabled).SetOnChange(isOn =>
                {
                    if (!isOn)
                        diffInfo.CustomData.Remove(JsonKeys.MapFile);
                    else
                        diffInfo.CustomData[JsonKeys.MapFile] = $"{difficulty}{characteristic}";
